#!/usr/bin/env python3
"""
Step 12: Model Calibration (Platt Scaling)

Applies Platt scaling to improve probability calibration and computes
Expected Calibration Error (ECE) for both layer configurations.
Also saves per-bin reliability data for Figure 5.

Usage:
    python pipeline/step_12_calibration.py --run-dir runs/2025-01-01_000000/

Outputs:
    - results/calibration/split40_calibration_layer33_mean.json
    - results/calibration/split40_calibration_layers20_30_mean.json
    - results/calibration/split40_calibration.json  (backward-compat, = layer33_mean)
    - models/lr_split40_calibrated.joblib
"""

import argparse
import json
import numpy as np
import pandas as pd
from pathlib import Path
from datetime import datetime
from sklearn.linear_model import LogisticRegression
from sklearn.calibration import CalibratedClassifierCV
from sklearn.metrics import accuracy_score, log_loss, f1_score
import joblib

import sys
sys.path.insert(0, str(Path(__file__).parent.parent))
from pipeline.membership import load_split

# Project root
PROJECT_ROOT = Path(__file__).parent.parent

# Configuration
RANDOM_STATE = 42
N_BINS = 10


def compute_ece(y_true, y_proba, n_bins=10):
    """Compute Expected Calibration Error."""
    confidences = np.max(y_proba, axis=1)
    predictions = np.argmax(y_proba, axis=1)

    if isinstance(y_true[0], str):
        unique_labels = sorted(set(y_true))
        label_to_idx = {l: i for i, l in enumerate(unique_labels)}
        y_true_idx = np.array([label_to_idx[l] for l in y_true])
    else:
        y_true_idx = np.array(y_true)

    accuracies = (predictions == y_true_idx)
    bin_boundaries = np.linspace(0, 1, n_bins + 1)
    ece = 0.0

    for i in range(n_bins):
        in_bin = (confidences > bin_boundaries[i]) & (confidences <= bin_boundaries[i + 1])
        prop_in_bin = in_bin.mean()
        if prop_in_bin > 0:
            avg_confidence = confidences[in_bin].mean()
            avg_accuracy = accuracies[in_bin].mean()
            ece += np.abs(avg_accuracy - avg_confidence) * prop_in_bin

    return float(ece)


def compute_reliability_bins(y_true, y_proba, n_bins=10):
    """Compute per-bin reliability data for reliability diagram."""
    confidences = np.max(y_proba, axis=1)
    predictions = np.argmax(y_proba, axis=1)

    if isinstance(y_true[0], str):
        unique_labels = sorted(set(y_true))
        label_to_idx = {l: i for i, l in enumerate(unique_labels)}
        y_true_idx = np.array([label_to_idx[l] for l in y_true])
    else:
        y_true_idx = np.array(y_true)

    accuracies = (predictions == y_true_idx)
    bin_boundaries = np.linspace(0, 1, n_bins + 1)
    bins = []

    for i in range(n_bins):
        in_bin = (confidences > bin_boundaries[i]) & (confidences <= bin_boundaries[i + 1])
        count = int(in_bin.sum())
        if count > 0:
            mean_conf = float(confidences[in_bin].mean())
            frac_pos = float(accuracies[in_bin].mean())
        else:
            mean_conf = float((bin_boundaries[i] + bin_boundaries[i + 1]) / 2)
            frac_pos = 0.0
        bins.append({
            "bin_lower": float(bin_boundaries[i]),
            "bin_upper": float(bin_boundaries[i + 1]),
            "mean_confidence": mean_conf,
            "fraction_positive": frac_pos,
            "count": count
        })

    return bins


def calibrate_config(config_name, emb_file, train_ids, test_ids, id_to_idx,
                     id_to_label, results_dir, models_dir, split_name):
    """Train uncalibrated and calibrated models for one embedding config."""
    print(f"\n{'='*60}")
    print(f"Config: {config_name}")
    print(f"{'='*60}")

    embeddings = np.load(emb_file)

    train_idx = [id_to_idx[uid] for uid in train_ids if uid in id_to_idx]
    test_idx  = [id_to_idx[uid] for uid in test_ids  if uid in id_to_idx]
    train_labels = [id_to_label[uid] for uid in train_ids if uid in id_to_label]
    test_labels  = [id_to_label[uid] for uid in test_ids  if uid in id_to_label]

    X_train = embeddings[train_idx]
    X_test  = embeddings[test_idx]

    # --- Uncalibrated model ---
    print("\nTraining uncalibrated model...")
    base_model = LogisticRegression(
        max_iter=1000,
        random_state=RANDOM_STATE,
        multi_class='multinomial',
        solver='lbfgs',
        class_weight='balanced'
    )
    base_model.fit(X_train, train_labels)

    y_pred_uncal  = base_model.predict(X_test)
    y_proba_uncal = base_model.predict_proba(X_test)

    acc_uncal = accuracy_score(test_labels, y_pred_uncal)
    f1_uncal  = f1_score(test_labels, y_pred_uncal, average='macro', zero_division=0)
    ll_uncal  = log_loss(test_labels, y_proba_uncal, labels=base_model.classes_)
    ece_uncal = compute_ece(test_labels, y_proba_uncal)
    rel_uncal = compute_reliability_bins(test_labels, y_proba_uncal)

    print(f"  Accuracy: {acc_uncal:.4f}  Macro-F1: {f1_uncal:.4f}  ECE: {ece_uncal:.4f}")

    # --- Calibrated model (Platt scaling) ---
    print("\nTraining calibrated model (Platt scaling)...")
    calibrated_model = CalibratedClassifierCV(base_model, method='sigmoid', cv=5)
    calibrated_model.fit(X_train, train_labels)

    y_pred_cal  = calibrated_model.predict(X_test)
    y_proba_cal = calibrated_model.predict_proba(X_test)

    acc_cal = accuracy_score(test_labels, y_pred_cal)
    f1_cal  = f1_score(test_labels, y_pred_cal, average='macro', zero_division=0)
    ll_cal  = log_loss(test_labels, y_proba_cal, labels=calibrated_model.classes_)
    ece_cal = compute_ece(test_labels, y_proba_cal)
    rel_cal = compute_reliability_bins(test_labels, y_proba_cal)

    print(f"  Accuracy: {acc_cal:.4f}  Macro-F1: {f1_cal:.4f}  ECE: {ece_cal:.4f}")

    # Save calibrated model (layer33_mean only — primary model)
    if config_name == "layer33_mean":
        model_file = models_dir / f"lr_{split_name}_calibrated.joblib"
        joblib.dump(calibrated_model, model_file)
        print(f"\n  ✓ Saved model: {model_file}")

    # Build report
    report = {
        "step": 12,
        "name": "Model Calibration",
        "config": config_name,
        "timestamp": datetime.now().isoformat(),
        "split": split_name,
        "method": "Platt scaling (sigmoid)",
        "n_cv_folds": 5,
        "n_bins_ece": N_BINS,
        "uncalibrated": {
            "accuracy": float(acc_uncal),
            "macro_f1": float(f1_uncal),
            "log_loss": float(ll_uncal),
            "ece": float(ece_uncal),
            "reliability_bins": rel_uncal
        },
        "calibrated": {
            "accuracy": float(acc_cal),
            "macro_f1": float(f1_cal),
            "log_loss": float(ll_cal),
            "ece": float(ece_cal),
            "reliability_bins": rel_cal
        },
        "improvement": {
            "accuracy_delta": float(acc_cal - acc_uncal),
            "ece_delta": float(ece_cal - ece_uncal)
        }
    }

    report_file = results_dir / f"{split_name}_calibration_{config_name}.json"
    with open(report_file, 'w') as f:
        json.dump(report, f, indent=2)
    print(f"  ✓ Saved: {report_file.name}")

    return report


def main():
    parser = argparse.ArgumentParser(description="Apply model calibration")
    parser.add_argument("--run-dir", type=str, required=True)
    args = parser.parse_args()

    run_dir     = Path(args.run_dir)
    results_dir = run_dir / "results" / "calibration"
    models_dir  = run_dir / "models"
    results_dir.mkdir(parents=True, exist_ok=True)
    models_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 60)
    print("Step 12: Model Calibration")
    print("=" * 60)

    # Load embedding IDs
    ids_file = run_dir / "embeddings" / "esm2_t33_650M" / "ids.txt"
    with open(ids_file) as f:
        embedding_ids = [line.strip() for line in f if line.strip()]
    id_to_idx = {uid: i for i, uid in enumerate(embedding_ids)}

    # Load labels
    labels_file = PROJECT_ROOT / "data" / "processed" / "labels.csv"
    labels_df   = pd.read_csv(labels_file)
    id_to_label = dict(zip(labels_df['uniprot_id'], labels_df['label_used_for_experiments']))

    split_name = "split40"
    train_ids  = load_split(split_name, "train", run_dir)
    test_ids   = load_split(split_name, "test",  run_dir)
    print(f"\nUsing {split_name}: {len(train_ids)} train, {len(test_ids)} test")

    configs = {
        "layer33_mean":    "domain_E001_layer33_mean.npy",
        "layers20_30_mean": "domain_E001_layers20_30_mean.npy",
    }

    reports = {}
    for config_name, filename in configs.items():
        emb_file = run_dir / "embeddings" / "esm2_t33_650M" / filename
        if not (emb_file.exists() or emb_file.is_symlink()):
            print(f"\n⚠ Skipping {config_name}: embedding file not found")
            continue
        report = calibrate_config(
            config_name, emb_file, train_ids, test_ids,
            id_to_idx, id_to_label, results_dir, models_dir, split_name
        )
        reports[config_name] = report

    # Backward-compatible alias: split40_calibration.json → layer33_mean results
    if "layer33_mean" in reports:
        compat_file = results_dir / f"{split_name}_calibration.json"
        # Build flat structure matching old format for downstream consumers
        r = reports["layer33_mean"]
        compat = {
            "step": 12,
            "name": "Model Calibration",
            "timestamp": r["timestamp"],
            "split": split_name,
            "method": r["method"],
            "n_cv_folds": r["n_cv_folds"],
            "n_bins_ece": r["n_bins_ece"],
            "uncalibrated_accuracy": r["uncalibrated"]["accuracy"],
            "uncalibrated_log_loss": r["uncalibrated"]["log_loss"],
            "uncalibrated_ece":      r["uncalibrated"]["ece"],
            "calibrated_accuracy":   r["calibrated"]["accuracy"],
            "calibrated_log_loss":   r["calibrated"]["log_loss"],
            "calibrated_ece":        r["calibrated"]["ece"],
        }
        with open(compat_file, 'w') as f:
            json.dump(compat, f, indent=2)
        print(f"\n✓ Backward-compat: {compat_file.name}")

    print("\n" + "=" * 60)
    print("Step 12 COMPLETE")
    print("=" * 60)
    for cfg, r in reports.items():
        print(f"\n  {cfg}:")
        print(f"    Uncalibrated → Calibrated accuracy: "
              f"{r['uncalibrated']['accuracy']:.4f} → {r['calibrated']['accuracy']:.4f}")
        print(f"    ECE: {r['uncalibrated']['ece']:.4f} → {r['calibrated']['ece']:.4f}")


if __name__ == "__main__":
    main()
